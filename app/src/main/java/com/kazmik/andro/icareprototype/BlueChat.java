...
{
    TextView myLabel,x,y,z,temp,bpm;
    EditText myTextbox;
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        bpm = (TextView)findViewById(R.id.tvbpm);
                                    ...
                                    handler.post(new Runnable()
                                    {
                                        public void run()
                                        {
                                            if(data.startsWith("temp=")) {
                                                temp.setText(data.substring(5,data.length()));
                                            }
                                             if(data.startsWith("x="))
                                            {
                                                x.setText(data.substring(2,data.length()));
                                            }
                                            if(data.startsWith("y="))
                                            {
                                                y.setText(data.substring(2,data.length()));
                                            }
                                            if(data.startsWith("z=")){
                                                z.setText(data.substring(2,data.length()));
                                            }
                                            if(data.startsWith("BPM = "))
                                            {
                                                bpm.setText(data.substring(6,data.length()));
                                            }
                                        }
                                    });
                                }
